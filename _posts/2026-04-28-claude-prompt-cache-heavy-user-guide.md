...
이 글은 그 할인을 실제로 받기 위한 가이드입니다. 먼저 캐시가 어떻게 동작하는지를 짧게 짚고, 5분과 1시간 중 어느 쪽을 골라야 하는지를 살펴본 뒤, 헤비 유저들이 자주 빠지는 함정인 하네스 패턴 안에서의 캐시 깨짐을 다룹니다. 마지막으로는 오늘 바로 워크플로에 적용해볼 수 있는 실천 원칙 7가지를 정리합니다.
...
캐시를 잘 쓰려면 자주 변하는 것을 뒤로 보내야 합니다. 안정성을 기준으로 보면 이렇습니다.

```
거의 변하지 않는 것    → 가장 앞 (도구 정의, 핵심 규칙)
세션마다 변하는 것     → 그 다음 (사용자 컨텍스트)
턴마다 변하는 것       → 마지막 (메시지)
매 요청 변하는 것       → 끝으로 보내거나 빼기 (타임스탬프 등)
```

가장 흔한 실수가 시스템 프롬프트 첫머리에 타임스탬프나 세션 ID를 박아 넣는 경우입니다. 매 요청 앞부분이 바뀌니 캐시가 단 한 번도 작동하지 못합니다. 시간 정보가 정말 필요하면 본문 끝이나 메시지 쪽으로 보내야 합니다.
...
캐시에 저장할 때는 정가보다 25% 또는 100% 비쌉니다. 1시간짜리는 5분짜리보다 정확히 두 배 비싼 거죠. 대신 두 번째 호출부터 회당 90% 할인이 들어옵니다. 그래서 **재사용 횟수가 일정 횟수를 넘기는 순간부터는 무조건 이득**이 되는 구조입니다.

#### [ 50K 프롬프트를 4번 부르면 얼마가 빠질까 ]

50K 토큰짜리 프롬프트를 한 세션에서 4번 재사용한다고 가정해봅시다. 캐시를 안 쓰면 매번 50K × 5달러 = 0.25달러씩, 네 번이면 1달러입니다. 5분 TTL로 캐시를 쓰면 첫 호출에서 0.31달러를 내고, 그다음부터는 회당 약 0.025달러로 떨어집니다. 합이 약 0.39달러로, 60% 가까이 깎입니다.

다만 이 절감은 5분 안에 다음 호출이 들어와야 받을 수 있습니다. 만료된 다음에 부르면 첫 호출 비용을 다시 내야 합니다. 그래서 TTL을 어느 쪽으로 고를지가 헤비 유저에게는 큰 결정이 됩니다.
...
매분 호출이 들어오는 활발한 챗봇이라면 이게 결정적인 장점입니다. 처음 한 번만 비용을 내고, 그 뒤로는 호출할 때마다 90% 할인된 가격으로 같은 캐시를 쓰는 셈입니다.
...
반면 1시간 TTL은 Slack 응답이나 컨텍스트 전환 시간을 그대로 흡수합니다. 처음에 두 배 비용을 내지만, 한 시간 안에 들어오는 호출은 모두 90% 할인됩니다. 손익분기인 2.2회는 헤비 유저의 사용 패턴에서 어렵지 않게 넘어갑니다.
...
가장 함정이 많은 게 동시 디스패치입니다. 같은 시스템 프롬프트로 다섯 개의 서브에이전트를 한꺼번에 보내면 직관적으로는 첫 호출이 캐시를 만들고 나머지 네 개가 그걸 받아 쓸 것 같지만, 실제로는 그렇지 않습니다. **캐시는 첫 응답이 시작된 다음에야 다른 호출이 읽을 수 있는 상태가 됩니다.** 그래서 동시에 발사된 다섯 개는 모두 첫 호출 비용을 따로 따로 치릅니다.
...
| 디스패치 방식 | 캐시 동작 |
|-------------|----------|
| 병렬 동시 발사 | 모두 첫 호출 비용 (캐시가 아직 준비 안 됨) |
| 순차 디스패치 | 두 번째부터 캐시 적중 가능 |
| Fork (부모 컨텍스트 상속) | 부모 캐시 재사용, 가장 효율적 |

NVIDIA Dynamo가 측정한 자료에서도 메인 에이전트는 캐시 적중률이 91%인데 같이 발사된 서브에이전트들은 79% 수준으로, 약 12%포인트 낮게 나옵니다. 이 차이는 거의 전부 첫 호출에서 캐시가 없는 상태로 시작하는 비용에서 옵니다.
