...
다만 한 가지 예외가 있습니다. **서브에이전트 디스패치(5장에서 자세히 다루는 하네스 패턴의 핵심 호출 방식)는 환경과 무관하게 5분 TTL로 처리**됩니다. 단발성 호출이라 1시간 쓰기 비용을 회수하기 어려운 패턴이라는 판단이 반영된 기본값으로 보입니다(이 역시 세션 JSONL 로그에서 디스패치 호출의 TTL이 5분으로 찍히는 것으로 확인했습니다).
...
이걸 피하는 가장 깔끔한 방법이 [fork](https://code.claude.com/docs/ko/sub-agents)입니다. 책을 새로 한 권 인쇄하는 게 아니라 부모 책 한 권을 여러 명이 같이 보는 셈입니다. fork는 부모의 도구 정의와 시스템 프롬프트를 그대로 물려받기 때문에 앞부분이 자동으로 같아지고, 부모의 캐시를 그대로 재사용합니다. 부모 캐시가 이미 만들어져 TTL 안에 살아 있는 동안 fork를 띄우면, 다섯 개를 띄워도 첫 fork만 캐시 저장 비용을 부담하고 나머지 네 개는 모두 1/10 가격으로 처리됩니다.
