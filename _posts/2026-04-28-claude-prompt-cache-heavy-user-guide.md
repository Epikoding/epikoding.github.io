...
숫자로 한번 보면 감이 옵니다. Opus 4.7 기준으로 입력 토큰은 100만 개당 5달러, 50K짜리 시스템 프롬프트를 100번 호출하면 입력에만 25달러가 나갑니다. 같은 내용이 매번 그대로 들어가는데도요. 이걸 제대로 캐시하면 그 50K가 두 번째 호출부터는 가격이 1/10로 떨어집니다.
...
캐시는 한마디로 **계산 결과의 보관함**입니다. 같은 입력이 또 들어오면 다시 계산하지 않고 보관함에서 꺼내 쓰는 구조죠. Claude가 토큰을 처리할 때는 내부 계산 결과를 토큰마다 만들어내는데, 이 계산이 비용의 큰 부분입니다. 같은 시스템 프롬프트가 또 들어오면 같은 결과를 똑같이 다시 만드는 셈인데요, 그 결과를 서버에 잠깐 저장해두고 두 번째 호출부터는 바로 꺼내 쓰는 메커니즘이 [키-값 캐시](https://platform.claude.com/docs/ko/build-with-claude/prompt-caching)(Key-Value cache, 이하 캐시)입니다.

이 보관함의 효과가 얼마나 큰지는 가격으로 드러납니다. 첫 호출에서 캐시에 저장할 때는 평소보다 살짝 비싸지만, 두 번째 호출부터는 정가의 10% 수준입니다. 매 호출마다 9할이 자동으로 빠지는 셈입니다.

```text
캐시 미사용 (매 호출):     50K × $5.00/M = $0.25
캐시 적중 (2차 호출부터):  50K × $0.50/M = $0.025 (1/10)
```
...
캐시도 똑같습니다. 두 호출의 앞부분이 같으면 그만큼은 보관함에서 꺼내 쓰지만, 한 글자라도 달라지는 순간부터는 다시 계산해야 합니다(엄밀히는 API 차원에서 여러 breakpoint를 잡아 일부 lookback이 가능하지만, Claude Code 같은 자동 캐싱 환경에서는 사실상 prefix 단위로 동작한다고 봐도 됩니다). 그 끊기는 지점을 [캐시 경계(breakpoint)](https://docs.bswen.com/blog/2026-04-01-prompt-caching-explained/)라고 부르는데요, API에서는 `cache_control` 마커로 명시하고, Claude Code 같은 환경에서는 시스템이 알아서 잡아줍니다.

이 규칙에서 자연스럽게 한 가지 원칙이 나옵니다. **변하지 않는 부분을 앞에 두고, 매번 달라지는 부분을 뒤에 두라**는 것입니다. static-first, dynamic-last라고 부르는데 이 글에서 가장 자주 등장할 표현이니 머릿속에 한 번만 새겨두면 됩니다.
...
반대로 **사용자가 이번 턴에 새로 입력한 메시지**는 그 호출에서는 캐시 대상이 아닙니다. 매 턴 새로 들어오는 부분이라 그 자체로 캐시 키가 될 수 없기 때문입니다. 다만 그 메시지가 한 번 처리되고 대화 이력에 들어가는 순간부터는, 다음 턴의 캐시에 함께 들어갑니다.
...
캐시가 가져다주는 변화는 두 가지입니다. **비용**과 **응답 지연**입니다. 캐시 읽기는 정가의 10% 수준이고, 응답 지연도 최대 85%까지 줄어듭니다. 9할이 자동으로 빠지는 셈이라 일단 캐시가 작동하기만 하면 이득입니다.

#### [ Opus 4.7 가격으로 보는 손익 ]

토큰 100만 개당 [가격](https://platform.claude.com/docs/ko/about-claude/pricing)을 정리하면 다음과 같습니다.
...
캐시에 저장할 때는 정가보다 25%(5분) 또는 100%(1시간) 비쌉니다. 대신 두 번째 호출부터는 회당 가격이 1/10로 떨어집니다. 결국 **몇 번 재사용해야 본전이 나오는가**가 핵심 질문이 됩니다.

[손익분기점](https://claudecodeguides.com/claude-prompt-caching-pricing-and-cost-savings/)은 단순한 공식으로 나옵니다.
...
캐시를 쓰면 첫 호출만 살짝 비싸고, 그다음부터 가격이 1/10로 떨어져 합이 1달러에서 0.39달러로 됩니다. 약 61% 절감입니다.
...
일반 환경에서는 기본이 5분입니다. 1시간을 쓰고 싶다면 Claude Code 설정 파일(`~/.claude/settings.json` 또는 프로젝트 단위 `.claude/settings.json`)의 `env` 객체에 다음 한 줄을 추가하면 됩니다.
...
다만 한 가지 예외가 있습니다. **서브에이전트 디스패치는 환경과 무관하게 5분 TTL로 처리**됩니다. 단발성 호출이라 1시간 쓰기 비용을 회수하기 어려운 패턴이라는 판단이 반영된 기본값으로 보입니다(이 역시 세션 JSONL 로그에서 디스패치 호출의 TTL이 5분으로 찍히는 것으로 확인했습니다).

#### [ 손익분기점은 둘 다 낮습니다 ]

5분 TTL은 단 1.4번만 재사용해도 본전이고, 1시간 TTL도 약 2.2번이면 이득으로 넘어갑니다. 진입 장벽 자체는 둘 다 낮은 편입니다.

문제는 이게 끝이 아니라는 점입니다. 5분 TTL에는 강력한 장점이 하나 더 있는데요, [**사용 시 무료로 자동 갱신된다**](https://platform.claude.com/docs/ko/build-with-claude/prompt-caching#when-to-use-the-1-hour-cache)는 점입니다. 캐시는 읽힐 때마다 보관 시간이 다시 5분으로 리셋됩니다. 그래서 5분 안에 호출이 꾸준히 들어오기만 하면 캐시는 사실상 무한히 유지됩니다.

다만 이 자동 갱신은 **5분 TTL의 고유 특성**입니다. 1시간 TTL은 hit해도 시간이 늘어나지 않고, 첫 저장 시점부터 1시간 뒤에 그대로 만료됩니다. 두 TTL의 강점은 같은 자리에서 나오지 않는다는 뜻입니다.

매분 호출이 들어오는 활발한 챗봇이라면 5분 TTL의 자동 갱신이 결정적인 장점입니다. 처음 한 번만 비용을 내고, 그 뒤로는 호출할 때마다 정가의 10% 가격으로 같은 캐시를 쓰는 셈입니다.
...
반면 1시간 TTL은 Slack 응답이나 컨텍스트 전환 시간을 그대로 흡수합니다. 처음에 두 배 비용을 내지만, 한 시간 안에 들어오는 호출은 모두 9할 절감됩니다. 손익분기인 2.2회는 헤비 유저의 사용 패턴에서 어렵지 않게 넘어갑니다. 1시간 TTL의 강점은 자동 갱신이 아니라 **평탄한 1시간 보장**에 있다는 점이 핵심입니다.
...
하네스 패턴(Harness Pattern)은 **오케스트레이터가 여러 서브에이전트를 부려서 결과를 만들고 평가하는 구조**를 말합니다. 가장 흔한 형태는 결과를 만드는 Generator와 그 결과를 평가하는 Evaluator를 따로 두고, 오케스트레이터가 두 에이전트 사이를 조율하는 방식입니다. Evaluator가 결과를 승인하면 완료고, 부족하면 피드백과 함께 오케스트레이터로 돌아가 재작업이 시작됩니다.
...
가장 함정이 많은 게 병렬 디스패치입니다. 같은 시스템 프롬프트로 다섯 개의 서브에이전트를 한꺼번에 보내면 직관적으로는 첫 호출이 캐시를 만들고 나머지 네 개가 그걸 받아 쓸 것 같지만, 실제로는 그렇지 않습니다. **캐시는 첫 응답이 시작된 다음에야 다른 호출이 읽을 수 있는 상태가 됩니다.** 그래서 동시에 디스패치된 다섯 개는 모두 첫 호출 비용을 따로 따로 치릅니다.

이걸 피하는 가장 깔끔한 방법이 [fork mode](https://code.claude.com/docs/ko/sub-agents#현재-대화-포크)의 사용입니다. 책을 새로 한 권 인쇄하는 게 아니라 부모 책 한 권을 여러 명이 같이 보는 셈입니다. fork는 부모의 도구 정의와 시스템 프롬프트를 그대로 물려받기 때문에 앞부분이 자동으로 같아지고, 부모의 캐시를 그대로 재사용합니다. 부모 캐시가 이미 만들어져 TTL 안에 살아 있는 동안 fork를 띄우면, 다섯 개를 띄워도 첫 fork만 캐시 저장 비용을 부담하고 나머지 네 개는 모두 1/10 가격으로 처리됩니다.

| 디스패치 방식           | 캐시 동작                      |
| ----------------- | -------------------------- |
| 병렬 디스패치           | 모두 첫 호출 비용 (캐시가 아직 준비 안 됨) |
| 순차 디스패치           | 두 번째부터 캐시 적중 가능            |
| fork (부모 컨텍스트 상속) | 부모 캐시 재사용, 가장 효율적          |
포크 모드의 사용은 아래와 같이 하실 수 있습니다.
```json
{
  "env": {
...
[NVIDIA Dynamo가 측정한 자료](https://docs.nvidia.com/dynamo/dev/blog/agentic-inference)에서도 메인 에이전트는 캐시 적중률이 91%인데 같이 디스패치된 서브에이전트들은 79% 수준으로, 약 12%포인트 낮게 나옵니다. 이 차이는 거의 전부 첫 호출에서 캐시가 없는 상태로 시작하는 비용에서 옵니다.
...
오케스트레이터는 매 호출 직전에 위의 고정 경로 파일에 새 데이터를 덮어씁니다. 서브에이전트는 짧은 메시지를 받고, Read 도구를 호출해 데이터를 가져옵니다. 여러 세션을 동시에 운용한다면 경로에 `<세션-id>`와 같은 한 단계 끼워 넣어(예: `~/.cache/agent/<세션-id>/source.md`) 세션 간 데이터 섞임을 피하는 게 안전합니다.
...
같은 50K 프롬프트, 같은 4회 재시도 시나리오를 두 방식으로 비교해봅니다. 변수 치환 방식은 매번 캐시가 깨지므로 4회 모두 정가입니다. 합계 약 1달러. 도구 호출 분리 방식은 메시지 본문이 4회 동안 그대로 유지되어 prefix가 안정적으로 캐시되고, 변하는 데이터는 도구 호출 결과로 들어가 별도 봉투로 처리됩니다. 첫 호출만 캐시 저장 비용을 내고 나머지 세 번은 캐시 읽기 가격이 되는 셈이라 합계 약 0.39달러로 떨어집니다.

대략 61% 절감입니다. 도구 호출이 한 번 더 들어가니 응답 시간이 3~5초 늘어나는 트레이드오프는 있지만, 비용과 캐시 안정성 면에서 의미 있는 개선입니다.

이게 Claude Code 추상화 안에서 헤비 유저가 캐시 경계를 의식적으로 다루는 가장 강한 레버 중 하나입니다. 캐시 경계를 직접 못 박는 환경에서, 메시지 본문을 작게 유지하고 변하는 데이터를 도구 호출로 빼는 것이 사용자가 가진 가장 강한 레버입니다.

### 6. 캐시 친화적 Claude 사용법 7가지

지금까지의 메커니즘을 헤비 유저가 자기 워크플로에 곧바로 적용할 수 있도록 7가지로 정리했습니다. 효과의 결을 미리 한 줄로 보면 다음과 같습니다.

```text
구조 차원 (큰 효과): 1. 세션 작게 → 3. fork → 4. static-first → 5. CLAUDE.md 보존
시간 차원:           2. TTL 안에 다음 호출
관찰 차원:           6. 모델 스위칭 자제 → 7. 메트릭 모니터링
```

#### [ 1. 세션 컨텍스트는 작게 유지하세요 ]

앞부분이 클수록 캐시 저장 비용도 큽니다. 50K 앞부분의 저장 비용이 0.31달러라면 100K로 늘어날 때 0.62달러로 정확히 두 배가 됩니다. 손익분기 호출 수 자체는 그대로지만, 한 번 손익분기를 넘기지 못하면 잃는 절대 비용이 두 배가 되는 셈입니다.

세션을 시작할 때 한 번만 자문해보면 좋습니다. 이 앞부분 안의 내용 중 정말로 매 호출에 필요한 게 어디까지일까. 안 쓰는 도구 정의를 빼거나, 지난 대화 이력을 요약으로 압축하는 게 캐시 효율을 올리는 가장 직접적인 방법입니다. 아니면 대화를 [이런 방법](https://epikoding.github.io/posts/how-i-record/#4-claude-conversation-extractor-%EB%8C%80%ED%99%94%EB%A5%BC-%EC%9E%90%EB%A3%8C%EB%A1%9C-%EA%BA%BC%EB%82%B4-%EC%98%A8%EB%8B%A4)으로 내보내고 새로운 세션을 시작에서 해당 내용을 주입해 사용하시는 것도 방법입니다.
...
부모 캐시가 살아 있는 동안 fork를 띄우면, 다섯 개를 띄워도 첫 fork만 캐시 저장 부담을 지고 나머지 네 개는 모두 정가의 10%로 처리되는 캐시 읽기입니다. 반면 다섯 개의 새 서브에이전트를 동시에 보내면 다섯 번 모두 캐시 저장 비용을 따로 냅니다. 비용 차이가 N배까지 벌어집니다.
...
#### [ 5. CLAUDE.md / AGENTS.md를 자주 손대지 마세요 ]

Claude Code는 워크스페이스의 `CLAUDE.md`, `AGENTS.md`를 자동 캐싱 대상으로 묶어서 사용합니다. 이 파일들은 헤비 유저의 워크플로에서 매 세션 앞부분의 큰 부분을 차지합니다.

여기에 자주 바뀌는 메모를 추가하지 않는 게 좋습니다. 한 번 쓰고 거의 변하지 않는 핵심 규칙만 남기는 게 캐시 친화적입니다. 자주 바뀌는 메모는 별도 파일(예: `~/.claude/notes/today.md`)로 빼두고, 필요한 시점에만 Read 도구로 가져오는 편이 좋습니다. CLAUDE.md 자체는 그대로 캐시되고, 변하는 부분만 메시지 쪽 도구 호출 결과로 들어오기 때문입니다.
...
Claude Code에서는 이 필드를 응답 화면에 직접 띄워주지 않습니다. 대신 세션 JSONL 로그(`~/.claude/projects/<프로젝트-경로>/<세션-id>.jsonl`)에 매 호출의 응답이 그대로 기록되어 있어, 거기서 두 필드 값을 직접 확인할 수 있습니다. 새 세션을 시작하면 그날의 jsonl 파일이 새로 만들어지니, 잠깐 들어가서 `cache_read_input_tokens` 값이 두 번째 호출부터 잡히는지 한 번 보면 캐시가 의도대로 작동하는지 바로 판가름됩니다. 의도와 실제가 어긋나면 비용이 조용히 새어나갑니다.
...
| 원칙 | 한줄 핵심 |
|------|----------|
| 1. 세션 컨텍스트 작게 | 앞부분이 작을수록 저장 비용과 실패 시 손실이 줄어든다 |
| 2. TTL 안에 다음 호출 | 5분/1시간 창 안에서 호출이 이어져야 캐시가 산다 |
| 3. fork 적극 활용 | 부모 캐시를 그대로 쓰니 N개 띄워도 첫 fork만 부담한다 |
| 4. static-first, dynamic-last | 변하지 않는 부분을 앞에, 변하는 데이터는 뒤로 |
| 5. `CLAUDE.md` / `AGENTS.md` 보존 | 자주 바뀌는 메모는 별도 파일로 분리한다 |
| 6. 모델 스위칭 자제 | 캐시는 모델별로 분리되어 전환마다 처음부터 다시 쓴다 |
| 7. 응답 메트릭 모니터 | `cache_read`/`cache_creation` 토큰이 진실의 근거다 |
