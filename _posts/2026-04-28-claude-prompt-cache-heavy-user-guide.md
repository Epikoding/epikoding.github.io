...
단, 여러 세션을 동시에 운용하는 헤비 유저라면 한 가지 함정이 있습니다. 모든 세션이 같은 `~/.cache/agent/source.md`를 공유하면 세션 A가 쓰는 사이 세션 B가 덮어써서, A의 서브에이전트가 B의 데이터를 읽어버리는 race condition이 생깁니다. 이걸 피하려면 세션마다 경로를 분리하면 됩니다. 예를 들어 `~/.cache/agent/<세션-id>/source.md`처럼 세션 식별자를 한 단계 끼워 넣으면, 한 세션 안에서는 그 식별자가 매번 같아 캐시 이점은 그대로 유지되고 세션 간 충돌만 빠집니다.
