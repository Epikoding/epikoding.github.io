...
오케스트레이터는 매 호출 직전에 위의 고정 경로 파일에 새 데이터를 덮어씁니다. 서브에이전트는 짧은 메시지를 받고, Read 도구를 호출해 데이터를 가져옵니다. 여러 세션을 동시에 운용한다면 이 경로를 세션별 namespace로 분리해 race condition을 피하는 게 안전합니다.
