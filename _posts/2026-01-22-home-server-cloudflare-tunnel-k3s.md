...
일반적인 웹 서버 운영은 **집 문을 열어놓고 손님을 기다리는 것**과 같다. 누구든지 문을 두드릴 수 있고, 원치 않는 방문자(해커)도 들어올 수 있다.
...
외부에서 내 서버로 직접 접속이 불가능하다. 모든 트래픽이 Cloudflare를 거치므로 내 서버의 방화벽에서 인바운드를 모두 막아도 된다.

**2. DDoS 보호**

Cloudflare는 전 세계에 분산된 엣지 서버 네트워크를 가지고 있다. 악의적인 트래픽을 엣지에서 필터링하고, 정상 트래픽만 내 서버로 전달한다.

**3. SSL/TLS 자동**

Cloudflare가 인증서를 자동으로 발급하고 갱신한다. Let's Encrypt 설정을 직접 할 필요가 없다.

**4. IP 숨김**

`dig` 명령어로 도메인을 조회해도 Cloudflare의 IP만 보인다. 내 서버의 실제 IP는 노출되지 않는다.
...
`config.yml`은 cloudflared에게 **"어떤 요청을 어디로 보내라"**라고 알려주는 **작업 지시서**다.
...
이 명령어를 실행하면 **Cloudflare DNS에 CNAME 레코드가 자동 생성**된다.
...
Traefik은 K3s에 기본 설치된 **Ingress Controller**다. 쉽게 말해 **똑똑한 교통 경찰**이다.
...
- 도메인 등록업체(Porkbun, 가비아)는 **소유권**만 관리한다
- 네임서버를 Cloudflare로 변경하면 **DNS 레코드 관리**는 Cloudflare가 한다
