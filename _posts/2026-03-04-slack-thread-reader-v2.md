...
쓰레드 안의 답글 링크를 복사해서 넘기면 엉뚱한 결과가 나오고, 채널 이름 대신 ID만 보여서 헷갈리며, 출력 형식도 LLM이 읽기에 적합하지 않았습니다. 이 글에서는 이런 문제들을 하나씩 해결하면서 스킬을 개선한 과정을 정리합니다.

이 스킬의 핵심 흐름은 링크 파싱에서 출력까지의 경로입니다. 안정성 개선이나 봇 메시지 처리 같은 부수적인 개선 사항은 이후 섹션에서 별도로 다룹니다.
...
답글 링크에 붙는 `thread_ts`는 "이 답글이 속한 쓰레드의 부모 메시지 ts"를 가리킵니다. `cid`는 채널 ID의 중복인데, Slack 네이티브 앱이 딥링크를 처리할 때 query string만으로 채널, 부모, 답글 세 가지를 모두 파악할 수 있도록 넣어둔 값이라 스크립트에서는 무시해도 됩니다.
...
이 코드에 답글 링크를 넘기면 path의 `p1770776063826049`(답글의 ts)를 쓰레드 루트로 사용해버립니다. 실제 부모인 `thread_ts=1770775935.866499`는 무시되기 때문에, `conversations.replies` API에 잘못된 ts가 전달되어 쓰레드 전체를 가져오지 못하는 거였습니다.

#### [ 어떻게 수정했나 ]

query string에서 `thread_ts`를 추출하는 로직을 추가했습니다. `thread_ts`가 있으면 답글 링크, 없으면 기존처럼 쓰레드 링크로 판단하는 방식입니다.
...
`--with-threads`를 붙이면 쓰레드가 있는 메시지의 답글까지 인라인으로 포함합니다.
...
| 옵션 | 설명 | 기본값 |
|------|------|--------|
| `--limit N` | 채널 모드에서 가져올 메시지 수 (0이면 전체) | 0 |
| `--from YYYY-MM-DD` | 이 날짜 이후 메시지만 | 없음 |
| `--to YYYY-MM-DD` | 이 날짜까지 메시지만 | 없음 |
| `--with-threads` | 쓰레드 답글을 인라인으로 포함 | off |
| `--thread-limit N` | 쓰레드당 최대 답글 수 (0이면 전체) | 0 |
...
도입부에서 언급한 "채널 이름 대신 ID만 보이는" 문제도 여기서 해결했습니다. `conversations.info` API로 채널 이름을 조회해서, 헤더에 ID와 함께 `#product-design`처럼 표시하도록 변경했습니다. 각 메시지에는 ISO 타임스탬프와 Slack ts(메시지 고유 ID)를 포함했고, 참여자 목록도 헤더에 넣었습니다.
...
출력 형식을 다듬다 보니, 첨부파일의 Slack permalink URL이 의외로 토큰을 많이 잡아먹고 있었습니다. 파일 하나당 80자 이상이 URL에 사용되는 경우가 흔했거든요.
...
유저 캐시까지 정리하고 나니, 테스트 중 발신자가 `?`로 표시되는 메시지가 눈에 띄었습니다. 알고 보니 봇 메시지였습니다.
...
아직 손대지 못한 부분도 남아 있습니다. 한 달치 채널을 수집했더니 메시지가 수백 건이었는데, 날짜별 구분 없이 쭉 이어지니 맥락이 바뀌는 지점을 파악하기 어려웠습니다. 봇이 남긴 입퇴장 알림이 전체의 상당 부분을 차지하는 것도 신경 쓰이는 부분입니다. 날짜별 그룹핑이나 시스템 메시지 필터링, 긴 메시지 truncate 같은 것들은 필요해지면 추가할 계획입니다.
