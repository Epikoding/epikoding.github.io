...
세 간극은 곧 세 원칙이 된다. 첫째, 쓰는 일을 시스템이 돕게 한다. 둘째, 원본을 길어 올리고 사람이 직접 검증할 수 있는 형태로 둔다. 셋째, 쌓인 기록이 서로 이어지도록 지식망에 편입시킨다. 네 개의 도구(Karabiner, claude-conversation-extractor, qmd, document 스킬)가 이 세 원칙을 어떻게 나누어 맡고 있는지를 먼저 한 장으로 조감하면 다음과 같다.
