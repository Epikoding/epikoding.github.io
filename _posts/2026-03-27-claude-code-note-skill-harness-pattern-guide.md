...
v3는 라운드당 Opus 3개 + Sonnet 8~9개로 구성된다. Opus의 토큰당 단가가 Sonnet보다 수배 높으므로, Opus를 생성과 주관적 판단(3개)에만 쓰고 나머지 평가를 Sonnet(8~9개)에 맡기는 구성은 상당한 비용 절감이 기대된다. 평가 관점은 오히려 6개에서 10개로 67% 확대되었다. "양 x 다양성 > 깊이 x 1" 원칙이 비용과 품질 양쪽에서 효과를 보인 것이다.
...
돌이켜보면, scope-filter는 v3에서 발생한 "기존 이슈가 재작업을 무한 발동"하는 문제를 막으려 도입한 장치였다. 그런데 이번에는 정반대 증상이 나타났다. 기존 이슈를 아예 무시하니 점수가 오르지 않는 것이다. 같은 원인(평가-수정 범위 불일치)의 서로 다른 증상이었던 셈이다.
...
교정 건수도 같이 봐야 한다. v2는 Projects 파일 28개에서 15건 교정(교정률 54%), v3는 Projects 파일 12개에서 5건 교정(교정률 42%)이었다. 절대 건수가 67% 줄었는데, 원인 후보는 두 가지다. 파일 수 자체가 줄어 검사 대상이 줄어든 효과, 그리고 규칙이 늘면서 만들어진 파일의 품질이 올라간 효과. 교정률도 54%에서 42%로 줄었으니 표본 축소만의 효과는 아니지만, 두 요인이 얼마나 기여했는지를 분리하기는 어렵다. 확실한 것은 규칙이 늘어나면서 Generator의 주의력이 "파일을 충분히 만드는 것"에서 "추가된 규칙을 지키는 것"으로 이동했다는 점이다. "규칙을 추가하는 것과 모든 규칙이 동시에 따라지는 것은 다르다"는, 프롬프트 엔지니어링에서 자주 간과되는 교훈이 여기서 드러났다.
...
그 위에 하네스 레이어의 구조적 한계도 드러났다. v4에서 Invariants에 dangling link 검사를 추가했지만 정작 Generator가 위키링크를 만들지 않으면 어떻게 될까? 검사할 대상 자체가 없다. 하네스 레이어는 Generator가 만든 결과물의 품질을 "검증"하는 도구이지, Generator가 무엇을 만들어야 하는지를 "생성"하는 도구가 아니기 때문이다. Evaluator가 "위키링크가 없다"고 감점할 수는 있어도, 위키링크를 만들어주지는 못한다.
...
가장 크게 실감한 것은, Evaluator를 만들었다고 끝이 아니라는 점이다. 회의적 튜닝 없이는 독립된 Evaluator라도 관대해지기 쉽고, 에이전트를 분리해야 각자의 프롬프트 안에서 주의력을 온전히 발휘할 수 있다. 생성과 평가를 한 프롬프트에 섞어놓았던 것이 근본 원인이었고, 이 둘을 분리하는 것만으로도 품질 개선과 유지보수가 훨씬 수월해졌다.
