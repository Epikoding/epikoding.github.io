...
v2에서는 4축을 6축으로 확장하면서 각 축에 전문 Evaluator를 할당했다. 여기에 2단계 평가(Content 단계 → Style 단계)를 도입해서 기술적 정확성이 확보되지 않은 상태에서 자연스러움을 손보는 토큰 낭비를 방지했고, CC도 8종으로 확장했다. UNCLEAR 메커니즘은 note에서 가져온 패턴으로, Evaluator가 판단이 모호한 지점을 삼키지 않고 표면화하는 장치다. 마지막으로 Final Check 단계를 추가해서, Content와 Style 각각의 수정이 다른 쪽을 훼손하지 않았는지 전체 축을 한 번 더 확인하는 구조를 갖추었다.
...
v3는 라운드당 Opus 3개 + Sonnet 8~9개로 구성되는데, v2(Opus 6개 + Sonnet 1개) 대비 약 35% 비용이 절감되었다. Opus를 줄이고 Sonnet을 늘렸기 때문인데, 평가 관점은 오히려 6개에서 10개로 67% 확대되었다. "양 x 다양성 > 깊이 x 1" 원칙이 비용과 품질 양쪽에서 효과를 보인 것이다.
...
모델 라우팅도 공통 패턴으로 자리 잡고 있다. `tech-blog-transformer` v3에서 Opus는 창작과 주관적 판단에, Sonnet은 관점 다양성이 필요한 평가에 배치했는데, 이는 vault-sync에서 Generator(생성)와 Evaluator(검증)를 분리한 것과 같은 결의 설계다. 역할에 맞는 모델을 쓰면 비용과 품질 양쪽에서 효율이 올라간다는 것을 세 스킬 모두에서 확인했다.
...
`tech-blog-transformer`에서 고유하게 배운 것은 세 가지다. 첫째는 Evaluator 회의적 튜닝의 중요성이다. 독립된 Evaluator를 두더라도 LLM의 기본 채점 성향은 관대한 편이어서, 합리화 금지, 문장 인용 강제, 점수 기준 교정 같은 구체적인 지침이 없으면 Evaluator가 Generator에게 너무 쉽게 합격점을 준다. 4축 채점 기준에 가중치를 부여해서 승인 임계값을 차등 적용한 것도 "모든 품질 요소가 동일하게 중요하지는 않다"는 현실을 반영한 설계였다.
...
셋째는 v3에서 얻은 Evaluator 다양성의 힘이다. 14에이전트 실험에서 확인한 "양 x 다양성 > 깊이 x 1" 원칙은 하네스 설계 전반에 걸쳐 유효한 통찰이었다. 같은 모델을 5개 돌리면 같은 이슈만 잡지만, 역할이 다른 모델 5개는 각각 다른 이슈를 잡는다. 이를 모델 라우팅(Opus 3개 + Sonnet 8~9개)으로 구현하자 비용은 35% 줄면서 평가 관점은 67% 늘어났다. 독자 수준 시스템도 v3의 주요 교훈인데, 입문/중급/고급을 파이프라인 첫 단계에서 한 번 선택하면 Generator의 톤, Evaluator의 채점 렌즈, 활성화 축까지 전체에 전파되어 글의 일관성이 높아졌다. 하나의 파라미터가 시스템 전체의 행동을 바꾸는 이 구조는, 각 에이전트에 독자 수준을 개별 전달하는 것보다 훨씬 관리하기 쉬웠다.
