...
description: 하네스 패턴(Generator/Evaluator 분리)의 개념과, Claude Code 스킬 세 개에 적용한 과정을 정리한다. note 스킬의 60개 문서 실험과 v3.2 재시도 루프 수정, v4.0의 Evaluator 분리(CC/Content/Structural)와 Intent Brief 도입, tech-blog-transformer의 6축 채점 기준과 Consistency Checker 사전 필터, vault-sync의 5레이어 아키텍처와 프롬프트 주의력 경제까지 다룬다.
mermaid: true
---
...
`note` 스킬의 리팩터링과 60개 문서 실험, v3.2 재시도 루프에서의 선언적 vs 절차적 제어 전환, v4.0에서 Evaluator를 CC/Content/Structural 세 에이전트로 분리한 과정, `tech-blog-transformer`의 6축 채점 기준과 Consistency Checker 사전 필터, 그리고 `vault-sync`의 5레이어 아키텍처와 4버전에 걸친 진화 과정까지 함께 살펴본다.
