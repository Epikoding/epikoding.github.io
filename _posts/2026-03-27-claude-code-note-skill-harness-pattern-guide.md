...
description: 하네스 패턴(Generator/Evaluator 분리)의 개념과, Claude Code 스킬 세 개에 적용한 과정을 정리한다. note 스킬의 60개 문서 실험과 v3.2 재시도 루프 수정, tech-blog-transformer의 4축 채점 기준과 Consistency Checker 사전 필터, vault-sync의 5레이어 아키텍처와 프롬프트 주의력 경제까지 다룬다.
mermaid: true
---
...
`note` 스킬의 리팩터링과 60개 문서 실험, v3.2 재시도 루프에서의 선언적 vs 절차적 제어 전환, `tech-blog-transformer`의 4축 채점 기준과 Consistency Checker 사전 필터, 그리고 `vault-sync`의 5레이어 아키텍처와 4버전에 걸친 진화 과정까지 함께 살펴본다.
...
`note` 스킬에서 가장 의미 있었던 발견은 60개 문서 실험이다. "LLM이 실제로 무엇을 틀리는가"를 감이 아니라 데이터로 확인하고 나니, 프롬프트에 남겨야 할 규칙과 빼도 되는 규칙을 구분하는 기준이 생겼다. 규칙이 많다고 좋은 게 아니라 정말 틀리는 규칙에 토큰을 집중시키는 것이 효과적이라는 걸 실감했다. v3.2 재시도 루프 수정에서는 또 다른 교훈을 얻었다. 선언적 지시("이렇게 해라")만으로는 재시도 시 같은 실수를 반복하기 쉽고, 절차적 제어("이 순서로 이것을 확인하라")로 전환하고 피드백을 구체화하며 재시도 모드를 분리해야 비로소 루프가 수렴한다는 것이다.
