...
각 에이전트가 2~3개 검사에만 집중하므로 주의력 분산이 줄고, 5개가 병렬로 실행되기 때문에 시간은 1개일 때와 동일하다. 이 구조는 vault-sync의 Evaluator 분리, note v4.0의 CC/Content/Structural 분리와 같은 원칙의 재적용이다. "에이전트 분리가 주의력 경제를 구조적으로 회피한다"는 것인데, 5절 vault-sync에서 배운 프롬프트 주의력 경제가 하나의 에이전트 안에서 적용되는 원칙이었다면, 에이전트를 분리하면 각 에이전트의 역할 범위 내에서 주의력 분산을 줄이는 셈이다.
