...
tags: [claude code, skill, harness pattern, generator evaluator, prompt engineering, vault-sync, evaluator centric, consistency checker, evaluator split, intent brief, unclear mechanism, model routing, evaluator diversity, reader level]
description: 하네스 패턴(Generator/Evaluator 분리)의 개념과, Claude Code 스킬 세 개에 적용한 과정을 정리한다. note 스킬의 60개 문서 실험과 v3.2 재시도 루프 수정, v4.0의 Evaluator 분리(CC/Content/Structural)와 Intent Brief 도입, tech-blog-transformer의 4축 채점에서 v3의 10축 평가 확장과 Opus/Sonnet 모델 라우팅, vault-sync의 5레이어 아키텍처와 프롬프트 주의력 경제까지 다룬다.
mermaid: true
---
...
`note` 스킬의 리팩터링과 60개 문서 실험, v3.2 재시도 루프에서의 선언적 vs 절차적 제어 전환, v4.0에서 Evaluator를 CC/Content/Structural 세 에이전트로 분리한 과정, `tech-blog-transformer`의 4축 채점에서 v3의 10축 평가 확장과 Opus/Sonnet 모델 라우팅, 그리고 `vault-sync`의 5레이어 아키텍처와 4버전에 걸친 진화 과정까지 함께 살펴본다.
...
| 공유 채점 기준(계약서) | Generator와 Evaluator 양쪽이 참조하는 평가 기준 문서 |
...
이 스킬에서는 `note`보다 한 단계 더 정교한 설계를 적용했다. 4-에이전트 아키텍처로 오케스트레이터(SKILL.md), Generator(`agents/generator.md`), Consistency Checker(`agents/consistency-checker.md`), Evaluator(`agents/evaluator.md`)를 분리하고, 채점 기준(`references/rubric.md`)을 Generator-Evaluator 공유 채점 기준(계약서)으로 설계했다.
...
v2로 이 블로그 글을 돌렸을 때 4라운드 만에 6축 모두 9점을 달성했다. 그런데 해당 글을 14개 서브에이전트(Opus 1개 + 역할이 다른 Sonnet 13개)로 별도 리뷰한 결과, 6축 9점을 받은 글에서 수십 개의 이슈가 발견되었다. v2 Evaluator가 잡아야 했지만 놓친 이슈도 있었고, 6축 어디에도 해당하지 않아서 애초에 커버할 수 없었던 이슈도 있었다.
...
Opus 1개가 깊이 있게 보는 것보다, 역할이 다른 Sonnet N개가 각자의 관점으로 보는 쪽이 확률적으로 더 많은 이슈를 잡았다. 확률 모델의 한계를 시행 횟수와 관점 다양성으로 보완하는 셈이다. 다만 생성(Generator)과 주관적 판단(자연스러움 평가, 반례 탐색)에는 Opus의 깊은 추론이 여전히 필요했다.
...
flowchart TB
    ...
    subgraph CC_PHASE["Phase 4: CC"]
        CC["Consistency Checker<br/>11종 검사 (Sonnet)"]:::sonnet
    end
...
v3는 라운드당 Opus 3개 + Sonnet 8~9개로 구성되는데, v2(Opus 6개 + Sonnet 1개) 대비 약 35% 비용이 절감되었다. Opus의 토큰당 단가가 Sonnet보다 수배 높으므로, Opus 에이전트 수를 절반으로 줄이고 Sonnet을 늘린 구성은 비용 효율적이다. 원본 문서 기준 약 35% 절감으로 추정된다. 평가 관점은 오히려 6개에서 10개로 67% 확대되었다. "양 x 다양성 > 깊이 x 1" 원칙이 비용과 품질 양쪽에서 효과를 보인 것이다.
...
채점 기준(계약서)을 정본 파일(references)로 설계하는 것도 세 사례 모두에서 잘 먹혔다. `note`에서는 templates, gotchas, directory-guide가, `tech-blog-transformer`에서는 rubric.md가, `vault-sync`에서는 evaluation-examples.md와 templates.md가 이 역할을 맡았고, 규칙이 변경되면 정본 한 곳만 수정하면 양쪽 에이전트 모두에 반영된다.

오케스트레이터를 얇게 유지하는 것도 공통된 교훈이었다. 세부 규칙은 references에, 에이전트별 지침은 prompts나 agents에 분리하고 나니, 오케스트레이터에는 흐름 제어만 남게 되어 유지보수가 훨씬 수월해진 것이다. vault-sync가 대표적인 사례인데, SKILL.md의 14-step 흐름만 남기고 세부 규칙을 references로 분리해둔 덕분에 v4에서 Generator 프롬프트만 v1으로 롤백하면서 하네스 레이어는 손대지 않는 최소 변경이 가능했다.

모델 라우팅도 공통 패턴으로 자리 잡고 있다. `tech-blog-transformer` v3에서 Opus는 창작과 주관적 판단에, Sonnet은 관점 다양성이 필요한 평가에 배치했는데, 이는 vault-sync에서 Generator(생성)와 Evaluator(검증)를 분리한 것과 같은 결의 설계다. 역할에 맞는 모델을 쓰면 비용과 품질 양쪽에서 효율이 올라간다는 것을 tech-blog v3에서 확인했다.
...
`tech-blog-transformer`에서 고유하게 배운 것은 세 가지다. 첫째는 Evaluator 회의적 튜닝의 중요성이다. 독립된 Evaluator를 두더라도 LLM의 기본 채점 성향은 관대한 편이어서, 합리화 금지, 문장 인용 강제, 점수 기준 교정 같은 구체적인 지침이 없으면 Evaluator가 Generator에게 너무 쉽게 합격점을 준다. 14에이전트 실험에서 확인한 것처럼, 특정 관점을 명시하지 않은 Opus Evaluator는 논리적 엄밀성에 예시까지 있는 이슈에도 9점을 부여한 반면, 역할이 부여된 Sonnet이 같은 이슈를 정확히 잡아냈다. 4축 채점 기준에 가중치를 부여해서 승인 임계값을 차등 적용한 것도 "모든 품질 요소가 동일하게 중요하지는 않다"는 현실을 반영한 설계였다.
...
셋째는 v3에서 얻은 Evaluator 다양성의 힘이다. 14에이전트 실험에서 확인한 "양 x 다양성 > 깊이 x 1" 원칙은 하네스 설계 전반에 걸쳐 유효한 통찰이었다. 같은 모델을 5개 돌리면 같은 이슈만 잡지만, 역할이 다른 모델 5개는 각각 다른 이슈를 잡는다. 이를 모델 라우팅(Opus 3개 + Sonnet 8~9개)으로 구현하자 비용은 35% 줄면서 평가 관점은 67% 늘어났다. 독자 수준 시스템도 v3의 주요 교훈인데, 입문/중급/고급을 파이프라인 첫 단계에서 한 번 선택하면 Generator의 톤, Evaluator의 채점 렌즈, 활성화 축까지 전체에 전파되어 글의 일관성이 높아졌다. 하나의 파라미터가 시스템 전체의 행동을 바꾸는 이 구조 덕분에, 에이전트별 독자 수준 불일치를 원천 차단하는 구조다.
...
하네스 패턴 자체는 아래 참고 자료의 Anthropic과 OpenAI 문서에서 이미 잘 설명하고 있으니, 적용을 고려한다면 해당 문서부터 살펴보면 된다. 다만 세 사례를 거치면서 가장 크게 실감한 것은, Evaluator를 만들었다고 끝이 아니라는 점이다. 회의적 튜닝 없이는 독립된 Evaluator라도 관대해지기 쉽고, 기계적 검증(CC, Mechanical Invariants)을 품질 평가 앞단에 배치해야 Evaluator가 진짜 판단에 집중할 수 있다. 패턴을 다른 스킬에 이전할 때도 "무엇을 가져올 것인가"보다 "무엇을 가져오지 않을 것인가"가 더 중요한 판단이었는데, note v4.0에서 tech-blog의 6축 분리를 그대로 복사하지 않고 Content만 분리한 것이 대표적이다. vault-sync의 v3 퇴보에서 배운 프롬프트 주의력 경제는 하네스 설계 전반에 걸쳐 유효한 교훈이고, tech-blog v3의 14에이전트 실험에서 배운 "양 x 다양성 > 깊이 x 1"은 Evaluator 설계의 핵심 원칙이 되었다. 결국 하네스의 진화 방향은, 레이어를 더 쌓는 것이 아니라 역할에 맞는 모델을 배치하고 각 에이전트의 관점을 다양화하는 쪽으로 수렴하고 있다.
