...
tags: [claude-code, skill, harness-pattern, generator-evaluator, prompt-engineering, vault-sync, evaluator-centric, consistency-checker, evaluator-split, intent-brief, unclear-mechanism, model-routing, evaluator-diversity, reader-level, cc-split, scope-filter, attention-economy, proactive-generation]
description: 하네스 패턴(Generator/Evaluator 분리)의 개념과, Claude Code 스킬 세 개에 적용한 과정을 정리한다. note 스킬의 60개 문서 실험과 v3.2 재시도 루프 수정, v4.0의 Evaluator 분리(CC/Content/Structural)와 Intent Brief 도입, tech-blog-transformer의 4축 채점에서 v3의 10축 평가 확장과 Opus/Sonnet 모델 라우팅, v3.1의 CC 5분할 병렬화와 scope-filter 에이전트 신설, v3.2의 scope-filter 제거와 Generator proactive 전환, vault-sync의 5레이어 아키텍처와 프롬프트 주의력 경제까지 다룬다.
mermaid: true
---
...
`note` 스킬의 리팩터링과 60개 문서 실험, v3.2 재시도 루프에서의 선언적 vs 절차적 제어 전환, v4.0에서 Evaluator를 CC/Content/Structural 세 에이전트로 분리한 과정, `tech-blog-transformer`의 4축 채점에서 v3의 10축 평가 확장과 Opus/Sonnet 모델 라우팅, v3.1의 CC 5분할 병렬화와 scope-filter 에이전트 신설, v3.2의 scope-filter 제거와 Generator proactive 전환, 그리고 `vault-sync`의 5레이어 아키텍처와 4버전에 걸친 진화 과정까지 함께 살펴본다.

### 1. 왜 하네스 패턴이 필요한가

#### [ 단일 패스 스킬의 한계 ]

세 스킬 모두 처음에는 단일 패스 방식이었다. 하나의 프롬프트가 글을 생성하고, 같은 프롬프트 안에서 품질까지 점검하는 구조인데, 이 방식을 써보면서 가장 크게 부딪힌 문제가 자기 평가 편향이다.
...
개선 자체도 어려웠기 때문에 문제가 더 심각했다. 생성과 평가가 하나의 프롬프트에 섞여 있으면, 생성 품질을 높이려고 프롬프트를 수정할 때 평가 기준까지 흔들린다. 반대도 마찬가지여서, 한쪽을 건드리면 다른 쪽이 깨지는 악순환이 되기 쉬웠다.

그렇다면 근본 원인은 무엇일까? 결국 생성과 평가를 분리하지 않는 한 각 역할에 집중할 수가 없다는 데 닿는다.

### 2. 하네스 패턴이란

#### [ Generator/Evaluator 분리 ]

위 문제를 겪으면서 찾은 해법이 하네스 패턴(Harness Pattern)이다. 에이전트가 자유롭게 생성하되 별도의 평가 단계에서 독립적으로 검증하는 구조인데, OpenAI는 이 개념을 "Harness engineering", Anthropic은 "Harness design"이라 부르며, 이 글에서는 하네스 패턴으로 통칭한다.

이걸 실제로 적용해보면, 글을 쓰는 에이전트(Generator)와 글을 평가하는 에이전트(Evaluator)의 컨텍스트를 완전히 분리하게 된다. Evaluator는 Generator가 어떤 프롬프트를 받았는지 모르는 상태에서 오직 결과물과 평가 기준만으로 판단하고, 이 독립성이 자기 평가 편향을 줄이는 구조적 조건이 된다. 다만 독립된 Evaluator도 관대하게 채점하는 경향이 있어, 회의적 튜닝이 함께 필요하다(4절에서 자세히 다룬다).
...
다섯 가지가 별개의 문제처럼 보이지만, 이 중 문서 형식 불일치, 디렉토리 판단 오류, Obsidian 문법 오류의 근본 원인은 단일 패스의 한계에 있다. 생성과 검증이 동시에 일어나면서 어느 쪽도 충분히 할 수 없었던 것이다. 트리거 범위 부족과 시나리오별 형식 부재는 단일 패스 자체의 문제라기보다 규칙 설계의 부재에서 비롯된 것으로, 별도로 보완이 필요했다.
...
실험 설계는 단순했다. obsidian-gotchas.md를 참조하지 않은 상태에서 LLM이 순수 지식만으로 Obsidian 마크다운을 생성하게 한 뒤, 각 규칙의 위반 여부를 자동 검사하는 것이다. Opus 에이전트 10개, Sonnet 에이전트 50개를 돌려서 총 60개 문서를 생성했다. Opus와 Sonnet의 비율이 1:5인 이유는 Opus의 토큰 단가가 훨씬 높아서 동일 비용으로 더 많은 표본을 확보하기 어렵기 때문이다. 다만 Opus 표본이 10개로 적기 때문에, Opus 위반율 수치는 경향을 보여주는 참고값으로 해석하는 편이 적절하다.
...
이 결과를 바탕으로 위반율 0% 항목 9개를 하단 "참고" 섹션으로 이동시키고 상세 예시를 제거했다. 실험 과정에서 사용한 메타 정보(위반율 수치, 60개 문서 등)도 함께 빼냈는데, Generator와 Evaluator가 알아야 할 것은 "이 규칙을 지켜라"이지 "왜 이 규칙이 존재하는지"가 아니기 때문이다.
...
뒤의 5절에서 다룰 `vault-sync` 스킬의 구현과 비교해보면 차이가 뚜렷하다. 차이는 세 군데에서 나타났다. vault-sync에서는 SKILL.md의 실행 순서(Step) 중 Step 6에서 오케스트레이터가 Evaluator 결과를 직접 읽고 FAIL/PASS를 판정한 뒤, FAIL이면 Step 4(Generator)를 명시적으로 재호출하는 구조인 반면, note에는 이런 절차적 흐름이 없었다. Evaluator의 피드백 구체성도 문제였다. vault-sync의 Evaluator는 감점 항목마다 구체적인 수정 전략을 반환하는 데 비해 note의 "수정 지시"는 추상적인 수준에 머물러 있었다. 한편 Generator 쪽에도 재시도 모드라는 개념 자체가 없었다. vault-sync의 Generator는 `{recent_corrections}` 입력을 받아서 증분 수정을 수행하는 경로가 명확한 반면, note의 Generator는 최초 생성과 재시도를 구분하지 않았던 것이다.
...
vault-sync에서 사용하는 추가 레이어(5절에서 상세히 다룬다)는 note에 도입하지 않았다. vault-sync는 매일 수십 개 파일을 무인 생성하는 cron이라 다층 방어가 필요하지만, note는 사용자가 호출할 때 1개 문서를 생성하는 도구여서 Evaluator 루프만으로 충분하다고 판단한 것이다. 다만 실제 FAIL 패턴이 축적되면 few-shot 예시 파일을 추후 추가할 여지는 있다.

수정 후 첫 실행에서 전 항목 PASS를 기록했다. 가장 큰 교훈은 선언적 지시("피드백을 전달한다")만으로는 LLM이 루프를 건너뛸 수 있다는 것이다. while 루프 의사코드 같은 절차적 제어가 필요했고, Evaluator 피드백의 구체성과 Generator 재시도 모드 분리까지 갖춰져야 비로소 루프가 수렴하는 셈이다.
...
- **Content 채점 관대**: 5점 만점 중 최소 3점이면 PASS여서, "핵심은 포함되었으나 세부사항 일부 누락"도 통과할 수 있었음
...
note에서 확인한 Evaluator 분리와 CC 사전 검사의 효과는, 같은 시기에 진행한 tech-blog-transformer에서도 비슷한 형태로 나타났다.

### 4. 사례 2: tech-blog-transformer(이하 tech-blog) 스킬

#### [ 문제 - 자기 평가 편향과 품질 천장 ]

`note`와 `tech-blog-transformer`는 거의 같은 시기에 하네스 패턴을 도입하면서 서로 영향을 주고받았다. note v4.0에서 Evaluator를 CC/Content/Structural로 분리한 경험이 tech-blog의 축 분리에 참고가 되었고, 반대로 tech-blog v2에서 검증한 2단계 평가와 UNCLEAR 메커니즘이 note v4.0의 설계에 반영되기도 했다. 마크다운 문서를 기술 블로그 포스트로 변환하는 이 스킬도 기존에는 최종 점검 단계에서 글을 쓴 에이전트가 자기 작업을 스스로 점검하는 방식이었고, Anthropic의 하네스 설계 문서에서 지적한 "자기 평가 편향"이 그대로 나타나는 구조였다.
...
Consistency Checker(이하 CC)는 Generator 출력의 기계적 정합성을 사전 검사하는 역할이다. 산술 검증, 명명 일관성, 수치 교차 검증, stale 참조 스캔, frontmatter-본문 정합성 등 8종의 기계적 검사를 수행하고, 하나라도 FAIL이면 Evaluator를 건너뛰고 Generator에 바로 피드백을 보낸다. 기계적 오류가 남아 있는 상태에서 품질 평가를 돌리면 토큰만 낭비되므로, CC가 먼저 걸러내는 것이다.
...
가중치는 종합 점수 계산에 쓰는 게 아니라, 승인 임계값에서 차등 적용하는 방식이다. 9점 기준은 8점에서 반복 실험한 결과, 사용자가 추가 수정 없이 발행할 수 있는 최소 수준이 9점이었기 때문에 설정한 것이다.
...
자연스러움이 1라운드 6점에서 4라운드 9점까지 올라가는 과정이 핵심이다. Evaluator가 종결어미 반복, 백과사전 톤, 접속 패턴 과다 사용 등을 문장 인용으로 지적하면, Generator가 해당 부분만 수정하는 루프가 4회 반복되면서 승인 기준(9점)에 도달했다. 하네스 패턴 없이 단일 패스로 생성했다면, 자동 재시도 루프가 없으므로 1라운드의 6점 수준에 머물렀을 가능성이 높다.
...
v2에서는 4축을 6축으로 확장하면서 각 축에 전문 Evaluator를 할당했다. 여기에 2단계 평가(Content 단계 → Style 단계)를 도입해서 기술적 정확성이 확보되지 않은 상태에서 자연스러움을 손보는 토큰 낭비를 방지했고, CC도 8종으로 정비했다. UNCLEAR 메커니즘은 Evaluator가 판단이 모호한 지점을 삼키지 않고 표면화하는 장치다. 마지막으로 Final Check 단계를 추가해서, Content와 Style 각각의 수정이 다른 쪽을 훼손하지 않았는지 전체 축을 한 번 더 확인하는 구조를 갖추었다.
...
14에이전트 실험을 토대로 가설로 세운 v3의 설계 원칙은 다음과 같다.
...
첫 번째 원칙을 모델 라우팅으로 구현한 것이 두 번째 원칙이다. Opus는 창작과 종합 판단이 필요한 3개 에이전트(Generator, 자연스러움 Evaluator, 반례 탐색 Evaluator)에만 사용하고, 나머지 8~9개 평가 에이전트는 Sonnet이 각자 다른 관점에서 검증한다. 세 번째 원칙은 v2에 없던 새 개념으로, 입문/중급/고급 독자 수준을 파이프라인 첫 단계에서 선택하면 Generator의 글쓰기 톤부터 Evaluator의 채점 렌즈까지 전체에 전파된다.
...
v3에서 오케스트레이터를 포함한 에이전트 수가 9개에서 12~13개로 늘어났고, v3.1에서 CC 5분할과 scope-filter 추가로 16~18개(신규 16~17, 업데이트 17~18)가 되었다. 아래 다이어그램은 v3.1 기준이다. 서브에이전트는 Opus 3개(Generator, 자연스러움, 반례 탐색)와 Sonnet 12~14개(CC 5개 + scope-filter + Content Evaluator 4개 + Style Evaluator 3개(자연스러움은 Opus) + 재현 시뮬레이터)로 구성된다.

v3.1은 신규 모드(처음 작성)와 업데이트 모드(기존 글 수정)로 동작한다. 신규 모드에서는 Generator가 가이드와 회고 두 스타일로 병렬 생성한 뒤 사용자가 하나를 선택하는 Phase 3(스타일 선택)을 거치고, 업데이트 모드에서는 이 단계를 건너뛴다.
...
> 위 다이어그램은 v3.1 기준이다. scope-filter는 v3.2에서 제거되었으며, 변경된 아키텍처는 아래 v3.2 서브섹션에서 다룬다.
{: .prompt-info }

채점 기준도 6축에서 10축으로 확장했다. 기존 6축(자연스러움, 구조적 완성도, 기술적 정확성, 논리적 엄밀성, 약속 이행(글에서 선언한 내용이 실제로 다뤄지는지), 포맷 준수)에 14에이전트 실험에서 발견한 4개 영역을 추가한 것이다.
...
v3는 라운드당 Opus 3개 + Sonnet 8~9개로 구성된다. Opus의 토큰당 단가가 Sonnet보다 수배 높으므로, Opus를 생성과 주관적 판단(3개)에만 쓰고 나머지 평가를 Sonnet(8~9개)에 맡기는 구성은 라운드 수가 유사하다면 비용이 줄어들 수 있다. 평가 관점은 오히려 6개에서 10개로 67% 확대되었다. "양 x 다양성 > 깊이 x 1" 원칙이 비용과 품질 양쪽에서 효과를 보인 것이다.

다만 실제 비용은 입력 토큰 양과 라운드 횟수에 따라 달라진다. 10축으로 확장하면서 재작업이 발동하는 축이 늘어나 라운드 수가 증가할 가능성도 있기 때문에, 라운드당 비용 절감이 전체 비용 절감으로 직결된다고 단정하기는 어렵다.
...
첫 번째는 CC의 주의력 분산이다. Sonnet 1개가 11종 검사를 순차 수행하다 보니 뒤쪽 검사(시각-본문 정합성 등)의 품질이 떨어지는 현상이 나타났다. 프롬프트에 규칙을 추가할수록 각 규칙에 대한 모델의 주의력이 분산되는 현상이, 이번에는 CC 안에서 발생한 것이다.
...
각 에이전트가 2~3개 검사에만 집중하므로 주의력 분산이 줄고, 5개가 병렬로 실행되기 때문에 시간은 거의 동일하다. 이 구조는 vault-sync의 Evaluator 분리, note v4.0의 CC/Content/Structural 분리와 같은 원칙의 재적용이다. 프롬프트에 규칙이 늘어나면 각 규칙에 대한 모델의 주의력이 분산되는 현상을 "프롬프트 주의력 경제"라 부르는데(5절 vault-sync에서 자세히 다룬다), 에이전트를 분리하면 각 에이전트의 역할 범위 내에서 이 주의력 분산을 줄이는 셈이다.
...
수십 개의 서브에이전트가 돌았는데 점수가 변하지 않는다는 것은, 시스템 어딘가에 구조적 병목이 있다는 뜻이다. 근본 원인을 추적해보니 **평가 범위와 수정 범위의 불일치**에 닿았다.

Evaluator는 글 전체를 보고 채점하기 때문에 기존 이슈까지 포함하여 7점을 매긴다. 그런데 scope-filter가 이 피드백을 받아 "이건 이번 업데이트 범위 밖"이라고 분류하면, 해당 이슈가 Generator에 전달되지 않는다. 결과적으로 Evaluator가 매 라운드 같은 이슈를 반복 지적하지만, Generator는 그 이슈를 수정할 기회를 얻지 못하면서 점수가 제자리걸음을 한 것이다.

돌이켜보면, v3는 Evaluator가 글 전체를 평가하고 Generator도 글 전체를 수정하면서 평가-수정 범위가 일치했지만, 그 결과 기존 이슈까지 재작업이 무한 발동하는 문제가 생겼다. scope-filter는 이 문제를 막으려 수정 범위를 축소한 장치였는데, 이번에는 평가 범위(전체)와 수정 범위(축소됨)가 어긋나면서 점수가 오르지 않는 정반대 증상이 나타났다. 두 경우 모두 평가 범위와 수정 범위를 올바르게 정렬하는 것이 해결 과제였다.

이 지점에서 접근 방식 자체를 전환해야 했다. v3에서 무한 루프가 돌았던 근본 원인은 Generator가 Evaluator 피드백에 reactive하게 한 줄씩 패치하면서 새 문제를 만든 것이었다. scope-filter로 피드백을 차단하는 대신, Generator가 처음부터 글 전체를 보고 proactively 기존 이슈까지 수정하면, Evaluator가 발견할 이슈의 상당수가 첫 패스에서 해소된다.

실제로 검증 과정 마지막에 "기존 이슈도 함께 수정해라"라고 지시하자, Generator가 한 번에 전부 수정했고 아무 부작용도 없었다. 처음부터 이렇게 했어야 했는데, scope-filter라는 에이전트를 추가하는 쪽으로 접근한 것이 우회였다.

v3.2에서 변경한 내용은 세 가지다.
...
세 번째 규칙은 검증 과정에서 발견한 "행 166 토끼굴"에서 비롯되었다. 하나의 행(오름차순 근거)을 5번이나 수정했는데, front truncation → recency bias → lost in the middle → 문장 분리 → 재결합으로 매번 Evaluator가 새로운 각도에서 지적한 것이다. 결국 이 행의 문제는 표현이 아니라 주장 자체의 근거가 약한 것이었다. 같은 행을 반복적으로 표현만 바꾸는 것은 증상 치료일 뿐이고, 근거가 약한 주장은 축소하거나 삭제하는 편이 루프를 수렴시키는 근본 해법이다.

아키텍처 변경을 다이어그램으로 보면, v3.1에서 CONTENT/STYLE/META의 FAIL 경로가 scope-filter를 거쳐 Generator로 향하던 구조가 v3.2에서는 직접 Generator로 향하는 구조로 단순화되었다.
...
v3.1 다이어그램에서 scope-filter 서브그래프가 사라지고, CONTENT/STYLE/META의 FAIL 경로가 Generator로 직행하는 것이 핵심 변경이다. Generator 노드에 "proactive 수정"이 추가된 것은 업데이트 모드에서 첫 패스부터 기존 이슈를 함께 수정하는 동작을 반영한다.

이 변경에서 가장 큰 교훈은 **에이전트를 추가하는 것이 항상 답은 아니라**는 점이다. scope-filter는 관심사 분리 원칙에 따른 깔끔한 설계였지만, 실제로는 문제의 근본 원인(평가-수정 범위 불일치)을 해결하지 못하고 증상만 바꿔놓았다. Generator의 동작 방식을 reactive에서 proactive로 전환하는 쪽이, 전용 에이전트를 추가하는 것보다 더 효과적인 해법이었다. 아키텍처도 16~18에이전트에서 16~17에이전트로 오히려 줄었다.

tech-blog에서는 사용자가 호출할 때만 실행되기 때문에 결과가 마음에 들지 않으면 직접 고치면 그만이었다. 그런데 이런 수동 보정이 불가능한 스킬이라면 어떨까?

### 5. 사례 3: vault-sync 스킬

#### [ vault-sync란 ]

`vault-sync`는 성격 자체가 다르다. 매일 03:30에 cron으로 자동 실행되는 오케스트레이터 스킬로, 업무 캘린더(Supabase)와 Slack(SQLite)에서 데이터를 수집하여 Obsidian vault의 Members, Projects, Daily, Weekly 문서를 자동 생성한다.
...
| 레이어 | 역할 |
|--------|------|
| Calibrated Evaluator | 4-criteria 채점 + few-shot 보정 + 반복 루프. 4개 기준은 Fidelity(원본 충실도), Completeness(누락 없음), Craft(문서 품질), Coherence(문서 간 일관성)다 |
| Correction Protocol | 교정 이력을 실행 간(cross-run, 어제 sync에서 교정한 내용을 오늘 sync가 참조)으로 축적하여 evaluator/generator에 환류 |
...
다이어그램 상단의 "14-step 오케스트레이터"는 vault-sync의 SKILL.md가 데이터 수집부터 문서 생성, 평가, 교정 이력 기록, vault 건강도 점검까지 14단계의 실행 순서로 구성되어 있다는 뜻이다. 매일 무인으로 실행되는 cron이기 때문에, 사람이 개입할 수 없는 각 단계를 세밀하게 나눠 오케스트레이터가 빠짐없이 순서대로 수행하도록 설계한 것이다.
...
교정 건수도 같이 봐야 한다. v2는 Projects 파일 28개에서 15건 교정(교정률 54%), v3는 Projects 파일 12개에서 5건 교정(교정률 42%)이었다. 절대 건수가 67% 줄었는데, 원인 후보는 두 가지다. 파일 수 자체가 줄어 검사 대상이 줄어든 효과, 그리고 규칙이 늘면서 만들어진 파일의 품질이 올라간 효과. 교정률 차이(54% → 42%)는 표본 크기, 파일 유형 분포, 실제 품질 향상의 세 요인이 혼재한 수치로, 이것만으로 품질이 개선되었다고 판단하기는 어렵다. 확실한 것은 규칙이 늘어나면서 Generator의 주의력이 "파일을 충분히 만드는 것"에서 "추가된 규칙을 지키는 것"으로 이동했다는 점이다. "규칙을 추가하는 것과 모든 규칙이 동시에 따라지는 것은 다르다"는, 프롬프트 엔지니어링에서 자주 간과되는 교훈이 여기서 드러났다.
...
한 가지 미해결 이슈가 있다. v1에서는 Daily 문서에 프로젝트 위키링크가 18건 생성되었지만, v4에서는 동일한 generate-files.md를 쓰는데도 0건이다. 동일 프롬프트라도 오케스트레이터가 subagent를 호출할 때 함께 전달하는 컨텍스트 양이 다르면 Generator의 행동이 달라진다는 뜻인데, 단순한 오케스트레이션이던 v1과 하네스 데이터가 추가된 v2~v4 사이에서 이 차이가 발생한 것으로 추정된다. 이 위키링크 가설은 아직 검증되지 않았고, 오케스트레이터 컨텍스트 전달량의 영향을 분리하여 실험할 과제로 남아 있다. 이처럼 개별 스킬마다 고유한 미해결 과제가 남아 있지만, 세 스킬을 관통하는 공통 패턴도 뚜렷하게 드러났다.

### 6. 세 사례에서 배운 것

#### [ 공통 패턴 ]

세 스킬에서 공통적으로 드러난 패턴을 정리해보자.

가장 큰 효과는 프롬프트를 독립적으로 개선할 수 있게 된 점이다. Generator의 프롬프트를 바꿔도 Evaluator의 기준은 그대로이고, 반대도 마찬가지여서, 한쪽을 수정할 때 다른 쪽이 깨질 걱정이 없어졌다. vault-sync에서는 이 독립성이 특히 빛을 발했는데, v4에서 Generator 프롬프트만 v1으로 롤백하면서 하네스 레이어는 그대로 유지할 수 있었던 것도 이 분리가 있었기에 가능한 일이었다.

그렇다면 채점 기준은 어디에 두어야 할까? 세 사례 모두에서 효과적이었던 답은 정본 파일(references)이다. `note`에서는 obsidian-gotchas.md를 60개 문서 실험 후 664행에서 211행으로 줄일 때 references 한 곳만 수정하면 Generator와 Evaluator 양쪽에 반영되었고, `tech-blog-transformer`에서는 rubric.md가 4축에서 10축으로 확장할 때도 같은 역할을 했다. `vault-sync`에서는 evaluation-examples.md의 few-shot 예시가 Evaluator 보정에 직접 쓰인 셈이다. 규칙이 변경되면 정본 한 곳만 수정하면 되기 때문에, 양쪽 에이전트가 즉시 동기화되는 단순함이 실제 유지보수에서 큰 차이를 만들었다.

오케스트레이터를 얇게 유지하는 것도 빼놓을 수 없는 교훈인데, 세부 규칙은 references에, 에이전트별 지침은 prompts나 agents에 분리하고 나면 오케스트레이터에는 흐름 제어만 남게 된다. 세 스킬 모두 이 구조를 따르고 있지만, vault-sync에서 가장 뚜렷하게 효과를 확인할 수 있었다. 오케스트레이터가 얇았기 때문에 v4에서 Generator 프롬프트만 v1으로 교체하는 수술이 가능했다. SKILL.md에 14-step 흐름만 남기고 세부 규칙을 references로 분리해둔 덕분에, Generator 프롬프트 롤백이 하네스 레이어에 영향을 주지 않았기 때문이다. note에서도 오케스트레이터(SKILL.md 185행)는 흐름 제어만 담당하고, tech-blog에서도 SKILL.md에는 Phase 순서와 에이전트 호출만 남아 있다.

참고로 세 스킬의 채점 스케일은 서로 다르다. note는 5점 만점, tech-blog는 10점 만점, vault-sync는 20점 만점이다. 자기 스킬에 적용할 때는 스케일을 먼저 정한 뒤 임계값을 설정하는 것이 좋다.

#### [ 같은 원칙, 다른 구현 ]

세 사례를 가로질러 보면, 동일한 원칙이 스킬의 특성에 따라 전혀 다른 형태로 구현되었다는 점이 흥미롭다.

**"주의력은 유한하다"는 원칙**의 구현 방식부터 다르다. note는 60개 문서 실험으로 "LLM이 실제로 틀리는 규칙"만 남겨서 프롬프트 자체를 가볍게 만드는 방식(규칙 경량화)을 택했고, tech-blog는 같은 문제를 에이전트 수를 늘려 해결했다. 10축 10개 Evaluator가 각각 자기 축 하나만 보도록 분리한 것이다. vault-sync는 두 접근을 모두 썼는데도 v3에서 퇴보를 겪었는데, 무인 cron이라는 특성상 규칙이 계속 추가되면서 경량화의 효과를 상쇄했기 때문이다. 결국 세 스킬이 같은 원칙에 다른 해법을 적용한 이유는 규칙의 증가 속도와 실행 빈도가 달랐기 때문이다.

**Evaluator 분리의 깊이**도 스킬 성격에 따라 갈렸다. note는 Content만 독립시키고 Format/Placement/Metadata는 하나로 묶었는데, 후자가 기계적 검증이 가능한 항목이기 때문이다. tech-blog는 주관적 품질 판단이 핵심이라 10축 전문 Evaluator까지 확장했고, 14에이전트 실험에서 Opus 1개가 9점을 부여한 이슈를 역할이 다른 Sonnet이 정확히 잡아낸 것은 모델의 능력 차이가 아니라 관점의 차이가 검출력을 결정한다는 뜻이었다. vault-sync는 Evaluator 자체보다 그 주변 레이어(Correction Protocol, Scorecard)를 두텁게 쌓았는데, 무인 실행에서는 Evaluator가 놓친 오류가 다음 실행에서 자기 복제되므로 실행 간 이력 축적이 단일 실행의 정밀도보다 중요했기 때문이다.

**재시도 루프의 제어 방식**에서도 차이가 뚜렷하다. note v3.2에서 선언적 지시만으로는 LLM이 루프를 건너뛰는 문제를 절차적 의사코드로 해결한 반면, vault-sync는 처음부터 14-step 오케스트레이터에 절차적 제어를 내장했다. 사용자가 지켜보는 note에서는 루프 실패가 즉시 눈에 띄지만, 무인 cron인 vault-sync에서는 실패가 조용히 묻힐 수 있어서 사전 방어를 더 강하게 설계한 것이다. 이처럼 같은 원칙이라도 "누가 실행하는가"에 따라 제어의 강도가 달라져야 했다.

이 세 가지 비교에서 드러나는 것은, 하네스 패턴에 정답 구조는 없다는 점이다. 에이전트 분리, 규칙 경량화, 절차적 제어라는 도구는 동일하지만, 어느 도구를 얼마나 강하게 적용할지는 스킬의 실행 맥락(유인/무인, 주관적/기계적, 단발/반복)이 결정한다.

tech-blog v3.2에서 scope-filter를 제거한 경험은 여기에 또 하나의 교훈을 더한다. **에이전트를 추가하는 것이 항상 해법은 아니다.** scope-filter는 관심사 분리 원칙에 부합하는 깔끔한 설계였지만, 실제로는 평가-수정 범위 불일치라는 근본 원인을 해결하지 못했다. 오히려 Generator의 동작 방식을 reactive에서 proactive로 바꾸는, 기존 에이전트의 행동 전환이 전용 에이전트 추가보다 효과적이었다. 이 사례는 "문제가 생기면 에이전트를 분리한다"는 접근이 항상 유효하지는 않으며, 때로는 에이전트 수를 줄이는 쪽이 답일 수 있음을 보여준다.

그 위에 하네스 레이어의 구조적 한계도 드러난 것으로 보인다. v4에서 Invariants에 dangling link 검사를 추가했지만 정작 Generator가 위키링크를 만들지 않으면 어떻게 될까? 검사할 대상 자체가 없다. 이 현상의 원인이 아직 특정되지 않았으므로(오케스트레이터 컨텍스트 전달량의 영향이라는 검증 대기 중인 가설만 있다) 단정하기는 어렵지만, 하네스 레이어가 결과물의 품질을 "검증"하는 데 강점이 있는 반면, Generator가 무엇을 만들어야 하는지를 "유도"하는 데는 한계가 있을 수 있음을 시사한다.

마지막으로, v1과 v4의 위키링크 차이(5절에서 다룬 미해결 이슈)에서 확인한 것처럼, 동일한 프롬프트라도 오케스트레이터가 subagent에 전달하는 추가 컨텍스트(corrections, 수집 데이터 등)에 따라 Generator의 행동이 달라진다. 정확한 원인은 아직 특정되지 않았고, 오케스트레이터 컨텍스트 전달량의 영향이라는 가설만 남아 있는 상태다.

### 7. 내 스킬에 하네스 패턴 도입하기

위 사례를 참고해서 자기 스킬에 하네스 패턴을 도입할 때, 스킬의 특성에 따라 설계 선택이 달라진다. 아래 테이블로 자기 스킬의 위치를 먼저 파악한 뒤, 그에 맞는 구조를 선택하면 된다.
...
| 평가 성격 | 기계적 (형식, 문법, 수치) | 주관적 (자연스러움, 논리) | 주관적 평가가 많을수록 Evaluator 분리 깊이를 늘림 |
| 실행 빈도 | 단발 / 저빈도 | 반복 / 고빈도 | 고빈도일수록 Correction Protocol 같은 실행 간 학습을 고려 |
| 규칙 증가 속도 | 안정적 | 계속 추가됨 | 규칙이 빠르게 늘면 에이전트 분리로 주의력 분산을 선제 방어 |

#### [ 도입 체크리스트 ]

1. **채점 기준(계약서)부터 작성한다.** Generator와 Evaluator가 참조할 정본 파일(rubric)을 먼저 만든다. 최소한 채점 축과 각 축의 승인 임계값을 정의해야 하는데, 4절에서 소개한 tech-blog의 4축(자연스러움/구조/정확성/포맷)과 9점 임계값이 그 예시다. 기준 없이 Evaluator를 만들면 평가가 매번 달라진다.
2. **단일 Evaluator로 시작한다.** note처럼 하나의 Evaluator가 전체를 평가하는 구조에서 출발한다. 처음부터 축을 세분화하면 오버엔지니어링이 된다.
3. **CC(기계적 검사)를 Evaluator 앞에 배치한다.** 형식 오류가 남은 상태에서 품질 평가를 돌리면 토큰이 낭비된다. 기계적으로 검증 가능한 항목은 사전 검사로 분리한다.
4. **품질 천장에 부딪히면 축을 분리한다.** 단일 Evaluator로 일정 점수 이상 오르지 않으면, tech-blog처럼 축별 전문 Evaluator를 도입한다.
5. **회의적 튜닝을 적용한다.** Evaluator 프롬프트에 합리화 금지, 문장 인용 의무, 점수 기준 교정 지침을 넣는다. 4절에서 소개한 회의적 튜닝 지침을 활용할 수 있다. 이것 없이는 독립된 Evaluator도 관대해진다.
6. **에이전트 추가 전에 자문한다.** "이 에이전트가 근본 원인을 해결하는가, 증상만 우회하는가?" scope-filter 사례처럼, 기존 에이전트의 행동 전환이 더 효과적일 수 있다.

---

세 스킬에 하네스 패턴을 적용한 과정을 정리했다.

가장 크게 실감한 것은, Evaluator를 만들었다고 끝이 아니라는 점이다. tech-blog v1에서는 Evaluator 없이 Quality Check가 형식적으로 흘러가면서 "괜찮지만 뛰어나지 않은" 수준에 머물렀는데, 회의적 튜닝을 적용한 뒤에는 자연스러움이 1라운드 6점에서 4라운드 9점까지 올라가는 개선이 나타났다. 합리화 금지, 문장 인용 의무, 점수 기준 교정 등의 지침이 Evaluator의 관대한 채점 경향을 보정한 셈이다. 결국 에이전트를 분리해야 각자의 프롬프트 안에서 주의력을 온전히 발휘할 수 있고, 생성과 평가를 한 프롬프트에 섞어놓았던 근본 원인을 해소하는 것만으로도 품질 개선과 유지보수가 훨씬 수월해진다.

동시에, tech-blog v3.2에서 scope-filter를 제거하면서 배운 것도 있다. 문제가 생기면 에이전트를 추가하고 싶은 유혹이 크지만, 근본 원인을 정확히 짚지 않은 채 에이전트를 늘리면 복잡도만 올라가고 문제는 형태를 바꿔 되풀이된다. Generator의 동작 방식을 reactive에서 proactive로 전환하는 쪽이, 전용 필터 에이전트를 추가하는 것보다 더 직접적인 해법이었다.

자기 스킬에 처음 적용한다면 note처럼 단일 Evaluator 구조부터 시작하고, 품질 천장에 부딪히면 tech-blog처럼 축을 분리하는 순서가 자연스럽다. 다만 에이전트를 분리할 때마다 "이 에이전트가 근본 원인을 해결하는가, 증상만 우회하는가"를 자문해볼 필요가 있다. 하네스 패턴 자체는 아래 참고 자료의 Anthropic과 OpenAI 문서에서 이미 잘 설명하고 있으니, 적용을 고려한다면 해당 문서부터 살펴보면 된다. 다만 이 글의 사례는 모두 개인 프로젝트의 마크다운 생성 스킬이므로, 다른 환경에서의 적용은 별도 검증이 필요하다.
