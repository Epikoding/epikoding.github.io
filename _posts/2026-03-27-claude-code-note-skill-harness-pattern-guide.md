...
v3는 라운드당 Opus 3개 + Sonnet 8~9개로 구성되는데, v2(Opus 6개 + Sonnet 1개) 대비 약 35% 비용이 절감되었다. Opus와 Sonnet의 토큰당 단가 차이가 약 5배이므로, Opus를 6개에서 3개로 줄인 효과가 Sonnet을 1개에서 8~9개로 늘린 비용 증가를 상쇄하고도 남는 구조다. 평가 관점은 오히려 6개에서 10개로 67% 확대되었다. "양 x 다양성 > 깊이 x 1" 원칙이 비용과 품질 양쪽에서 효과를 보인 것이다.
...
원인을 추적해보니, generate-files.md의 프롬프트 길이와 구조 변경이 함께 작용한 결과였다. 프롬프트가 v2의 약 93줄에서 v3의 약 155줄로 67% 증가하면서, 슬랙 링크 조합 규칙, 재실행 처리, 실패 처리, Wikilink 경로 검증 등 새 규칙이 파일 생성 지시 앞뒤로 끼어들어 Generator가 본래 해야 할 파일 생성에 충분한 주의를 기울이지 못한 것이다.

흥미로운 점은 교정 건수가 오히려 67% 감소했다는 것이다(v2 15건에서 v3 5건). 파일 수가 줄었는데 교정도 줄었다면 모순처럼 보이지만, 두 현상은 프롬프트 주의력 트레이드오프의 양면이다. 규칙이 늘어나면서 Generator의 주의력이 "파일을 충분히 만드는 것"에서 "추가된 규칙을 지키는 것"으로 이동한 결과, 만들어진 파일의 품질은 올라갔지만(교정 감소) 만드는 파일 수 자체가 줄어든 것이다. 다만 교정 건수 감소에는 파일 수 자체가 줄어 검사 대상이 줄어든 효과도 영향을 미쳤을 가능성이 있다. v2가 28개 파일에서 15건 교정(교정률 54%)이었고 v3는 12개 파일에서 5건 교정(교정률 42%)이므로, 절대 건수 감소의 상당 부분은 표본 축소에서 비롯된 셈이다. "규칙을 추가하는 것과 모든 규칙이 동시에 따라지는 것은 다르다"는, 프롬프트 엔지니어링에서 자주 간과되는 교훈이 여기서 드러났다.
...
셋째는 v3에서 얻은 Evaluator 다양성의 힘이다. 14에이전트 실험에서 확인한 "양 x 다양성 > 깊이 x 1" 원칙은 하네스 설계 전반에 걸쳐 유효한 통찰이었다. 같은 모델을 5개 돌리면 같은 이슈만 잡지만, 역할이 다른 모델 5개는 각각 다른 이슈를 잡는다. 이를 모델 라우팅(Opus 3개 + Sonnet 8~9개)으로 구현하자 비용은 35% 줄면서 평가 관점은 67% 늘어났다. 독자 수준 시스템도 v3의 주요 교훈인데, 입문/중급/고급을 파이프라인 첫 단계에서 한 번 선택하면 Generator의 톤, Evaluator의 채점 렌즈, 활성화 축까지 전체에 전파되어 글의 일관성이 높아졌다. 하나의 파라미터가 시스템 전체의 행동을 바꾸는 이 구조 덕분에, 에이전트별 독자 수준 불일치를 원천 차단할 수 있었다.
