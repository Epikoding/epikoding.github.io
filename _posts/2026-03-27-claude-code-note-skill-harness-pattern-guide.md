...
위 문제를 겪으면서 찾은 해법이 하네스 패턴(Harness Pattern)이다. 에이전트가 자유롭게 생성하되 별도의 평가 단계에서 독립적으로 검증하는 구조다. OpenAI는 이 개념을 "Harness engineering", Anthropic은 "Harness design"이라 부르는데, 이 글에서는 하네스 패턴으로 통칭한다.
...
각 에이전트가 2~3개 검사에만 집중하므로 주의력 분산이 줄고, 5개가 병렬로 실행되기 때문에 시간은 1개일 때와 동일하다. 이 구조는 vault-sync의 Evaluator 분리, note v4.0의 CC/Content/Structural 분리와 같은 원칙의 재적용이다. 프롬프트에 규칙이 늘어나면 각 규칙에 대한 모델의 주의력이 분산되는 현상을 "프롬프트 주의력 경제"라 부르는데(5절 vault-sync에서 자세히 다룬다), 에이전트를 분리하면 각 에이전트의 역할 범위 내에서 이 주의력 분산을 줄이는 셈이다.
...
앞의 두 스킬은 사용자가 호출할 때 한 번 실행되는 도구여서, 결과물이 마음에 들지 않으면 직접 고치면 그만이었다. `vault-sync`는 성격 자체가 다르다. 매일 03:30에 cron으로 자동 실행되는 오케스트레이터 스킬로, 업무 캘린더(Supabase)와 Slack(SQLite)에서 데이터를 수집하여 Obsidian vault의 Members, Projects, Daily, Weekly 문서를 자동 생성한다.
...
한 가지 미해결 이슈가 있다. v1에서는 Daily 문서에 프로젝트 위키링크가 18건 생성되었지만, v4에서는 동일한 generate-files.md를 쓰는데도 0건이다. 동일 프롬프트라도 오케스트레이터가 subagent를 호출할 때 함께 전달하는 컨텍스트 양이 다르면 Generator의 행동이 달라진다는 뜻인데, 단순한 오케스트레이션이던 v1과 하네스 데이터가 추가된 v2~v4 사이에서 이 차이가 발생한 것으로 추정된다. 이 위키링크 가설은 아직 검증되지 않았고, 오케스트레이터 컨텍스트 전달량의 영향을 분리하여 실험할 과제로 남아 있다.

### 6. 세 사례에서 배운 것

#### [ 공통 패턴 ]

세 스킬 각각의 구현과 미해결 이슈(vault-sync의 위키링크 가설 등)까지 살펴봤으니, 이제 공통적으로 드러난 패턴을 정리해보자.
...
1. **채점 기준(계약서)부터 작성한다.** Generator와 Evaluator가 참조할 정본 파일(rubric)을 먼저 만든다. 기준 없이 Evaluator를 만들면 평가가 매번 달라진다.
...
5. **회의적 튜닝을 적용한다.** Evaluator 프롬프트에 합리화 금지, 문장 인용 의무, 점수 기준 교정 지침을 넣는다. 이것 없이는 독립된 Evaluator도 관대해진다.
...
가장 크게 실감한 것은, Evaluator를 만들었다고 끝이 아니라는 점이다. tech-blog v1에서는 Evaluator 없이 Quality Check가 형식적으로 흘러가면서 "괜찮지만 뛰어나지 않은" 수준에 머물렀다. 회의적 튜닝을 적용한 뒤에는 자연스러움이 1라운드 6점에서 4라운드 9점까지 올라가는 개선이 나타났는데, 이는 합리화 금지, 문장 인용 의무, 점수 기준 교정 등의 지침이 Evaluator의 관대한 채점 경향을 보정한 결과로 볼 수 있다. 에이전트를 분리해야 각자의 프롬프트 안에서 주의력을 온전히 발휘할 수 있다. 생성과 평가를 한 프롬프트에 섞어놓았던 것이 근본 원인이었고, 이 둘을 분리하는 것만으로도 품질 개선과 유지보수가 훨씬 수월해졌다.
